    ...
    with tab1:
        ...
        with c4:
            ...
            try:
                ...
                # Encode Smoking_Status and GENDER
                input_df["Smoking_Status"] = input_df["Smoking_Status"].map({"Current smoker": 2, "Former smoker": 1, "Never smoker": 0})
                ...
                fig = go.Figure(data=[go.Pie(labels=feature_names, values=feature_importance, hole=0.4)])
                ...
                if health_score >= 80 and prediction == 0:
                    st.success("✅ Great alignment between health score and heart risk!")
                elif health_score < 60 and prediction == 1:
                    st.error("🚨 Low health score + High risk. Immediate doctor consultation advised.")
                elif health_score >= 80 and prediction == 1:
                    st.warning("⚠️ Good health score, but risk detected. Recommend full check-up.")
                elif health_score < 60 and prediction == 0:
                    st.info("🟡 Low health score but low risk. Focus on improving wellness habits.")
            except Exception as e:
                st.error(f"Model Error: {e}")

        st.markdown("### 🛡️ Preventive Measures")
        if latest["BMI"] < 18.5 or latest["BMI"] > 25:
            st.write(f"• BMI is {latest['BMI']} – Adjust diet and exercise.")
        if latest["Heart_Rate"] > 90:
            st.write("• High Heart Rate – Improve cardiovascular fitness and reduce stress.")
        if latest["Systolic_BP"] > 130 or latest["Diastolic_BP"] > 85:
            st.write("• Blood Pressure high – Reduce sodium and monitor BP regularly.")
        if str(latest["Smoking_Status"]).lower().startswith("current"):
            st.write("• Smoking – Recommended cessation to reduce heart disease risk.")
        if latest["Hyperlipidemia"]:
            st.write("• Hyperlipidemia – Heart-healthy diet, exercise, and possible medication.")
        if latest["Diabetes"]:
            st.write("• Diabetes – Maintain glucose levels and follow a diabetic-friendly diet.")
    ...
    with tab2:
        ...
        for _, row in patient_df.iterrows():
            ...
            if row["BMI"] < 18.5 or row["BMI"] > 25:
                tips.append("• Maintain BMI with a healthy diet and regular exercise.")
            if row["Heart_Rate"] > 90:
                tips.append("• Control high heart rate through cardio and stress management.")
            ...
            if row["Hyperlipidemia"]:
                tips.append("• Monitor cholesterol through a fiber-rich diet.")
            if row["Diabetes"]:
                tips.append("• Manage diabetes under doctor supervision.")
